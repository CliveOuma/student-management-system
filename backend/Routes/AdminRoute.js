...
router.post('/add_student', (req, res) => {
    const sql = `INSERT INTO students (name, admno, course, unit, score) VALUES (?, ?, ?, ?, ?)`;
...
router.put('/edit_student/:id', (req, res) => {
    const id = req.params.id;
    const sql = 'UPDATE students SET name=?, admno=?, course=?, unit=?, score=? WHERE id = ?';
    const values = [
        req.body.name,
        req.body.admno,
        req.body.course,
    ...
    con.query(sql, values, (err, result) => {
        ...
        // Ensure the response includes a clear 'Status' property
        return res.json({ status: true, result: result });
    });
});
